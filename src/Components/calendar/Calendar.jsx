...
const Calendar = (selectedMood, lastColor) => {
  ...
  const renderDates = (year, month) => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        let txtColor = "#000000"; // Default text color
        ...
        if (userData && Array.isArray(userData)) {
            ...
            if (userDataForDate) {
                ...
                if (moodColor === "#0000ff" || moodColor === 'rgb(0, 0, 255)') {
                    txtColor = "#ffffff"; // Set text color to white if background is #0000ff
                }
            }
        }

        // Apply selected mood color to current day
        if (selectedMood && day === currentDay && month === currentMonth && year === currentYear) {
            moodColor = selectedMood.selectedMood.color;
            if (moodColor === "#0000ff" || moodColor === 'rgb(0, 0, 255)') {
                txtColor = "#ffffff"; // Ensure text color is white when mood color is #0000ff
            }
        }

        const backgroundColor = selectedMood.selectedMood.name === "default" ? lastColor : moodColor;

        dates.push(
            <div key={`day-${day}`} className="date" style={{ backgroundColor, color: txtColor }}>
                {day}
            </div>
        );
    }
  ...
  //     if (selectedMood && day === currentDay && month === currentMonth && year === currentYear) {
  //       moodColor = selectedMood.selectedMood.color;
  //       // setLastColor(selectedMood.selectedMood.color);
  //       if (selectedMood.selectedMood.color == "#0000ff") {
  //         txtColor = "#ffffff";
  //       }
  //     }

  //     if(selectedMood.selectedMood.name==="default"){
  //     dates.push(
  //       <div key={`day-${day}`} className="date" style={{ backgroundColor: lastColor, color: txtColor }}>
  //         {day}
  //       </div>
  //     );
  //   }
